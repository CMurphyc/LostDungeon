...
namespace Pathfinding {
	...
	public abstract class AIBase : VersionedMonoBehaviour {
		...
		/// See: <see cref="Move"/>
		/// </summary>
		public Vector3 position { get { return updatePosition ? Position : simulatedPosition; } }

		/// <summary>
		/// Rotation of the agent.
		/// If <see cref="updateRotation"/> is true then this value is identical to transform.rotation.
		/// </summary>
		public Quaternion rotation { get { return updateRotation ? Rotation : simulatedRotation; } }
		...
		protected Vector3 RaycastPosition (Vector3 position, float lastElevation) {
			...
			float rayLength = LocalScale.y * height * 0.5f + Mathf.Max(0, lastElevation-elevation);
		...
		protected virtual void OnDrawGizmos () {
			...
			if (orientation == OrientationMode.YAxisForward) {
				Draw.Gizmos.Cylinder(position, Vector3.forward, 0, radius * LocalScale.x, color);
			} else {
				Draw.Gizmos.Cylinder(position, rotation * Vector3.up, LocalScale.y * height, radius * LocalScale.x, color);
			}
