...
public class MonsterModule
{
        ...
        foreach (int RoomID in _parentManager._player.GetLiveRoom())
        {
            if (RoomToMonster.ContainsKey(RoomID))
            {
                ...
                for (int i = 0; i < MonsterList.Count; i++)
                {
                    ...
                    if (Target != null)
                    {
                        Monster.GetComponent<AIDestinationSetter>().Target = new Vector3((float)Target.GetComponent<PlayerModel_Component>().Position.x, (float)Target.GetComponent<PlayerModel_Component>().Position.y, (float)Target.GetComponent<PlayerModel_Component>().Position.z);
    ...
    GameObject FindClosePlayer(Vector2 MonsterPos, int RoomID )
    {
        ...
        {
            Vector2 PlayerPos = new Vector2 ((float)PlayerInRoomList[i].obj.GetComponent<PlayerModel_Component>().Position.x, (float)PlayerInRoomList[i].obj.GetComponent<PlayerModel_Component>().Position.y);
